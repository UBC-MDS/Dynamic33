...
test_that("eda produces correct outputs", {
  ...
  contingency_table <- "tests/mock-contingency-table.csv"
  ...
  # Test 2: Ensure contingency table file is created
  expect_true(file_exists(contingency_table))

  # Test 3: Check level table structure and row count
  ...
  # Test 4: Ensure heatmap file is created
  expect_true(file_exists(heatmap_file))

  # Cleanup: Remove generated files after the test
  file_delete(c(level_table, heatmap_file, contingency_table))
})